    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {filteredAndSortedPlayers.map((player) => (
          <div key={player.id} className="glass-card p-4 flex items-center justify-between group hover:border-brand-accent/50 transition-all duration-300 relative overflow-hidden">
            <div className="flex items-center gap-4 relative z-10">
              {/* Avatar Section */}
              <div className="w-16 h-16 rounded-xl bg-brand-bg/50 border border-white/5 flex items-center justify-center overflow-hidden relative group-hover:bg-brand-accent/10 transition-colors">
                <img 
                  src={player.avatar} 
                  alt={player.name}
                  className="w-12 h-12 object-contain filter drop-shadow-[0_0_8px_rgba(255,255,255,0.3)] group-hover:scale-110 transition-transform duration-500"
                />
                <div className={`absolute bottom-0 right-0 w-3 h-3 rounded-full border-2 border-brand-bg ${player.gender === 'boy' ? 'bg-blue-400' : 'bg-pink-400'}`}></div>
              </div>
              
              {/* Info Section */}
              <div className="flex flex-col">
                <span className="text-lg font-bold text-white group-hover:text-brand-accent transition-colors">{player.name}</span>
                <div className="flex items-center gap-3 mt-1">
                  <div className="flex items-center gap-1.5 px-2 py-0.5 rounded bg-white/5 border border-white/5">
                    <span className="text-[10px] font-black text-gray-500 uppercase">Lv.</span>
                    <span className="text-sm font-bold text-white">{player.level}</span>
                  </div>
                  <div className="w-1 h-1 bg-gray-700 rounded-full"></div>
                  <span className={`text-xs font-bold ${player.gender === 'boy' ? 'text-blue-400/80' : 'text-pink-400/80'}`}>
            ...
            {/* Stats Grid */}
            <div className="flex items-center gap-4 relative z-10">
              <div className={`flex flex-col items-center px-3 transition-opacity duration-300 ${sortBy === 'captures' ? 'opacity-100' : 'opacity-40 group-hover:opacity-100'}`}>
                <span className="text-[9px] font-black text-brand-pokemon-gold uppercase tracking-tighter">Captures</span>
                <span className="text-lg font-black text-white">{player.captures}</span>
              </div>
              <div className="w-[1px] h-8 bg-white/5"></div>
              <div className={`flex flex-col items-center px-3 transition-opacity duration-300 ${sortBy === 'fishingLevel' ? 'opacity-100' : 'opacity-40 group-hover:opacity-100'}`}>
                <span className="text-[9px] font-black text-blue-400 uppercase tracking-tighter">Fishing</span>
                <span className="text-lg font-black text-white">{player.fishingLevel}</span>
              </div>
            </div>
